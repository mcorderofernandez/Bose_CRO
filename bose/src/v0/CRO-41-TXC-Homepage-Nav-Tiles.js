(function() {
    ...
    function customGoals() {
        ...
        // Category Scroll below Hero  v0 
        // Fires on v0 when a user scrolls through the categories below the homepage hero in v0.
        var nav = document.querySelector('.secondary-navigation__nav');
        nav.addEventListener("scroll", (event) => {
            track('v0_category_scroll');
        });
        ...
        var navControls = document.querySelectorAll('.tns-controls button');
        for (var i = 0; i < navControls.length; i++) {  
            navControls[i].addEventListener('click', (event) => {
                track('v0_category_scroll');
            });
        }
